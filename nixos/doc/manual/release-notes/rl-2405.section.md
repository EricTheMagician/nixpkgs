...
- The `conda` package has been updated to 24.1.2 and is now built from source. This package no longer relies on entering a shell to install miniconda but running `conda init` is still required.
  ...
  This can be overriden at build time by modifying the `defaultEnvPath` and `defaultPkgPath` attributes respectively or at run time by setting the `CONDA_ENVS_PATH` and `CONDA_PKGS_DIRS` environment variable.
