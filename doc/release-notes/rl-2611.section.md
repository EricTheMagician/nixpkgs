...
- [`pake`](https://github.com/tw93/pake), a tool that turns any webpage into a lightweight Rust/Tauri desktop app, was added. It also provides `pake.mkApp`, which builds a single webpage into a self-contained desktop package (executable, `.desktop` entry and icon) suitable for `environment.systemPackages`. See [](#sec-pake) for details.
